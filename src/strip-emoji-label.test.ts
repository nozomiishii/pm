...
describe("stripEmojiLabel", () => {
  // 単純な絵文字プレフィックスを除去する
  test("strips a simple emoji prefix", () => {
    expect(stripEmojiLabel("🪴 brain")).toBe("brain");
  });

  // 肌色(暗)の ZWJ シーケンスを除去する
  test("strips dark skin-tone ZWJ sequence", () => {
    expect(stripEmojiLabel("🧙🏿‍♂️ dotfiles")).toBe("dotfiles");
  });

  // 肌色(明)の ZWJ シーケンスを除去する
  test("strips light skin-tone ZWJ sequence", () => {
    expect(stripEmojiLabel("👨🏻‍🚀 dev")).toBe("dev");
  });

  // 異体字セレクタ付き絵文字を除去する
  test("strips satellite emoji with variation selector", () => {
